...
class CompanyEmbeddingAE(nn.Module):
    def __init__(self, T, F, M, latent_dim):
        ...
        # Fusion
        self.to_latent = nn.Linear(2 * latent_dim, latent_dim)
    ...
    def forward(self, x_fin, x_macro):
        ...
        h_fin = self.fin_encoder(x_fin.reshape(B, -1))
        h_macro = self.macro_encoder(x_macro.reshape(B, -1))

        z = self.to_latent(torch.cat([h_fin, h_macro], dim=-1))
